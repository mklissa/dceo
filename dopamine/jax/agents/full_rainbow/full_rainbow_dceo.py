...
class Option:
  def __init__(self, online_params, target_network_params, optimizer_state):
    ...
    self.target_network_params = target_network_params
...
def select_action(network_def, params, state, rng, num_actions, eval_mode,
  ...
  params = params if cur_opt is None else options[cur_opt].online_params
  rng, action = act(
    network_def, params, state, rng, num_actions, eval_mode, support, epsilon)
  return rng, action, cur_opt, option_term
...
class JaxFullRainbowAgentDCEO(dqn_agent.JaxDQNAgent):
  ...
  def __init__(self,
    ...
    self.option_prob = option_prob * int(num_options > 0)
    self.dur = option_duration
    self.rep_dim = rep_dim
    coeff_vector = onp.arange(rep_dim, 0, -1)
    coeff_vector = onp.concatenate((coeff_vector, onp.zeros(1)))
    self.cur_opt = None
    self.option_term = True
    self.option_training_steps = [0] * num_options
    self._log_transform = log_transform

    if preprocess_fn is None:
      self.rep_network_def = rep_network_def = rep_network(
        num_actions=rep_dim)
      self.preprocess_fn = networks.identity_preprocess_fn
    else:
      self.rep_network_def = rep_network_def = rep_network(
        num_actions=rep_dim, inputs_preprocessed=True)
      self.preprocess_fn = preprocess_fn

    def get_rep(params, state):
        return rep_network_def.apply(params, state)

    self._get_rep = jax.jit(get_rep)

    square_div_dim = lambda x : x**2 / rep_dim
    norm_transform = jnp.log1p if self._log_transform else square_div_dim
    ...
    def neg_loss_fn(phi_u, phi_v):
      ...
      for dim in range(rep_dim, 0, -1):
        ...
        loss += coeff * (
          dot_product ** 2 - norm_transform(x_norm)  - norm_transform(y_norm)  )
      return loss
  ...
  def _build_networks_and_optimizer(self):
    ...
    for o in range(self.num_options):
      ...
      online_params = self.network_def.init(rng, x=self.state,
                                              support=self._support)
      optimizer_state = self.optimizer.init(online_params)
  ...
  def _training_step_update(self):
    ...
    all_states = self._rep_sample_from_replay_buffer()

    (self.rep_optimizer_state, self.rep_params,
     pos_loss, neg_loss) = self._train_rep(
        self.rep_params, self.optimizer, 
        self.rep_optimizer_state, all_states)

    ### Train each of the options ###
    for o in onp.random.choice(self.num_options, 1, replace=False):
      ...
      loss_weights = self.get_loss_weights()
      (option.optimizer_state, option.online_params,
       _, _, self._rng) = train(
          self.network_def, option.online_params, option.target_network_params,
          self.optimizer, option.optimizer_state, states,
          self.replay_elements['action'], next_states,
          intr_rew, 
      ...
      if self.option_training_steps[o] % self.target_update_period == 0:
        self._sync_option_weights(option)
      self.option_training_steps[o] += 1
    ...
    if (self.summary_writer is not None and
      ...
      with self.summary_writer.as_default():
        ...
        tf.summary.scalar('PositiveLoss', pos_loss,
                          step=self.training_steps)
        tf.summary.scalar('NegativeLoss', neg_loss,
                          step=self.training_steps)
      self.summary_writer.flush()
      if hasattr(self, 'collector_dispatcher'):
        self.collector_dispatcher.write(
            [statistics_instance.StatisticsInstance(
              'Rainbow Loss', onp.asarray(mean_loss), step=self.training_steps),
             statistics_instance.StatisticsInstance(
              'Pos Loss', onp.asarray(pos_loss), step=self.training_steps),
             statistics_instance.StatisticsInstance(
              'Neg Loss', onp.asarray(neg_loss), step=self.training_steps),
             ],
  ...
  def _sync_option_weights(self, option):
    ...
    option.target_network_params = option.online_params
  ...
  def _rep_sample_from_replay_buffer(self,):
    ...
    next_states = self.preprocess_fn(self.replay_elements['next_state'])
    self._sample_from_replay_buffer()
    states_u = self.preprocess_fn(self.replay_elements['state'])
    self._sample_from_replay_buffer()
    states_v = self.preprocess_fn(self.replay_elements['state'])
    return onp.vstack((states, next_states, states_u, states_v))
  ...
  def begin_episode(self, observation):
    ...
    self._rng, self.action, self.cur_opt, self.option_term = select_action(
        ...
        self.epsilon_fn, self._support, self.option_term, self.option_prob,
        self.dur, self.cur_opt, self.num_options, self.options)
